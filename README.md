...
**Phone Workflows 不是只为微信设计的插件。**它由两层组成：

1. **通用手机能力层**：负责 Android 设备发现与选择、ADB/Appium 后端、OCR、Hermes Helper、权限检查、操作串行化和一次性确认机制。这一层不绑定微信。
2. **应用工作流适配层**：把通用手机能力封装成面向具体应用、带明确业务边界的工具。当前 `0.1.x` 只完成了微信适配，因此现阶段可见的应用工具以 `wechat_` 开头。
